...
class FechaDao:
    ...
    @classmethod
    def seleccionar_fecha_rango(cls,fecha_inicio,fecha_fin):
        with Cursordelpool() as cursor:
            cursor.execute(cls._SELECCIONAR_FECHA_RANGO, (fecha_inicio,fecha_fin))
    ...
    @classmethod
    def seleccionar_fecha_solo_dia(cls, fecha):
        with Cursordelpool() as cursor:
            cursor.execute(cls._SELECCIONAR_FECHA_DIA, (fecha,))
            registros_torta = cursor.fetchall()
            registro_habitos_torta = []
            for registro in registros_torta:
                habito = Habitos(tipo_habito=registro[0])
